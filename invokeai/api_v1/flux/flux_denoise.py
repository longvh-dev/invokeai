...
from invokeai.backend.flux.extensions.instantx_controlnet_extension import (
    InstantXControlNetExtension,
)
from invokeai.backend.flux.extensions.regional_prompting_extension import (
...
class FluxDenoise:
    ...
    #     default=True, description="Add noise based on denoising start."
    # )
    ...
    #     default=None,
    # )
    # positive_text_conditioning: FluxConditioningField | list[FluxConditioningField] = (
    #     InputField(description=FieldDescriptions.positive_cond, input=Input.Connection)
    # )
    # negative_text_conditioning: (
    #     FluxConditioningField | list[FluxConditioningField] | None
    # ) = InputField(
    #     default=None,
    #     description="Negative conditioning tensor. Can be None if cfg_scale is 1.0.",
    #     input=Input.Connection,
    # )
    # cfg_scale: float | list[float] = InputField(
    #     default=1.0, description=FieldDescriptions.cfg_scale, title="CFG Scale"
    # )
    ...
    #     default=1024, multiple_of=16, description="Width of the generated image."
    # )
    # height: int = InputField(
    #     default=1024, multiple_of=16, description="Height of the generated image."
    # )
    ...
    # )
    # seed: int = InputField(
    #     default=0, description="Randomness seed for reproducibility."
    # )
    ...
    #     input=Input.Connection,
    # )
    def __init__(self, transformer: TransformerField):
        self.transformer = transformer

    @torch.no_grad()
    def flux_denoise(
        self,
        positive_conditioning: List[FluxTextConditioning],
        negative_conditioning: Optional[List[FluxTextConditioning]] = None,
        width: int = 1024,
        height: int = 1024,
        num_steps: int = 4,
        guidance: float = 4.0,
        ...
        cfg_scale: Union[float, List[float]] = 1.0,
        cfg_scale_start_step: int = 0,
        cfg_scale_end_step: int = -1,
        denoising_start: float = 0.0,
        denoising_end: float = 1.0,
        add_noise: bool = True,
        controlnet_extensions: List[
            Union[XLabsControlNetExtension, InstantXControlNetExtension]
        ] = None,
        pos_ip_adapter_extensions: List[XLabsIPAdapterExtension] = None,
        neg_ip_adapter_extensions: List[XLabsIPAdapterExtension] = None,
    ) -> torch.Tensor:
        """Run the denoising process with a FLUX transformer model.

        Args:
            latents (torch.Tensor): The input latents tensor.
            mask (torch.Tensor): The inpaint mask tensor. Values of 0.0 represent the regions to be fully denoised, and
                1.0 represent the regions to be preserved.
        ...
        if width % 16 != 0 or height % 16 != 0:
            raise ValueError("Chiều rộng và chiều cao phải là bội số của 16.")
        if denoising_start < 0 or denoising_end > 1 or denoising_start > denoising_end:
            raise ValueError(
                "denoising_start và denoising_end phải từ 0 đến 1, start <= end."
            )
        if mask is not None and latents is None:
            raise ValueError("Mask yêu cầu latents ban đầu.")

        # Đặt giá trị mặc định
        controlnet_extensions = controlnet_extensions or []
        pos_ip_adapter_extensions = pos_ip_adapter_extensions or []
        neg_ip_adapter_extensions = neg_ip_adapter_extensions or []

        # Xác định loại mô hình
        is_schnell = "schnell" in self.transformer.config.config_path

        # Chuẩn bị nhiễu
        noise = get_noise(1, height, width, self.device, self.dtype, seed)

        # Chuẩn bị latents ban đầu
        if latents is not None:
            latents = latents.to(device=self.device, dtype=self.dtype)
            if add_noise:
                timesteps_full = get_schedule(
                    num_steps, (height // 16) * (width // 16), shift=not is_schnell
                )
                t_0 = timesteps_full[0]
                x = t_0 * noise + (1.0 - t_0) * latents
            else:
                x = latents
        else:
            if denoising_start > 0:
                raise ValueError("denoising_start phải là 0 khi không có latents.")
            x = noise

        # Tạo lịch trình timestep
        timesteps = get_schedule(
            num_steps, (height // 16) * (width // 16), shift=not is_schnell
        )
        timesteps = timesteps[
            max(0, int(len(timesteps) * denoising_start)) : min(
                len(timesteps), int(len(timesteps) * denoising_end) + 1
            )
        ]
        ...
        if mask is not None:
            ...
            inpaint_extension = InpaintExtension(
                init_latents=pack(latents),
                inpaint_mask=pack(mask),
                noise=pack(noise),
            )

        # Chuẩn bị regional prompting
        packed_h, packed_w = height // 16, width // 16
        pos_regional_prompting_extension = (
            RegionalPromptingExtension.from_text_conditioning(
                positive_conditioning, packed_h * packed_w
            )
        )
        neg_regional_prompting_extension = (
            RegionalPromptingExtension.from_text_conditioning(
                negative_conditioning, packed_h * packed_w
            )
        ...
        # Tạo img_ids
        img_ids = generate_img_ids(height // 8, width // 8, 1, self.device, self.dtype)

        # Chuẩn bị cfg_scale
        num_steps_actual = len(timesteps) - 1
        cfg_scale_list = (
            [cfg_scale] * num_steps_actual
            if isinstance(cfg_scale, float)
            else cfg_scale
        )
        if len(cfg_scale_list) != num_steps_actual:
            raise ValueError("Độ dài cfg_scale phải bằng số bước.")
        start_idx = min(max(cfg_scale_start_step, 0), num_steps_actual - 1)
        end_idx = (
            num_steps_actual - 1
            if cfg_scale_end_step == -1
            else min(max(cfg_scale_end_step, 0), num_steps_actual - 1)
        )
        clipped_cfg_scale = [1.0] * num_steps_actual
        clipped_cfg_scale[start_idx : end_idx + 1] = cfg_scale_list[
            start_idx : end_idx + 1
        ]
        ...
        x = denoise(
            ...
            timesteps=timesteps,
            guidance=guidance if not is_schnell else 1.0,
            cfg_scale=clipped_cfg_scale,
    ...
    def _run_diffusion(
        ...
        init_latents = (
            context.tensors.load(self.latents.latents_name) if self.latents else None
        )
        ...
        timesteps = clip_timestep_schedule_fractional(
            timesteps, self.denoising_start, self.denoising_end
        )
        ...
        pos_image_prompt_clip_embeds, neg_image_prompt_clip_embeds = (
            self._prep_ip_adapter_image_prompt_clip_embeds(
    ...
    def _load_text_conditioning(
        ...
        cond_list = (
            [cond_field]
            if isinstance(cond_field, FluxConditioningField)
            else cond_field
        )
        ...
        for cond_field in cond_list:
            ...
            text_conditionings.append(
                FluxTextConditioning(t5_embeddings, clip_embeddings, mask)
            )
    ...
    def _prep_controlnet_extensions(
        ...
        controlnet_extensions: list[
            XLabsControlNetExtension | InstantXControlNetExtension
        ] = []
        for controlnet, controlnet_cond in zip(
            controlnets, controlnet_conds, strict=True
        ):
            model = exit_stack.enter_context(
                context.models.load(controlnet.control_model)
            )
            ...
            if isinstance(model, XLabsControlNetFlux):
                ...
                if (
                    controlnet.instantx_control_mode is not None
                    and controlnet.instantx_control_mode >= 0
                ):
                    instantx_control_mode = torch.tensor(
                        controlnet.instantx_control_mode, dtype=torch.long
                    )
    ...
    def _prep_structural_control_img_cond(
        ...
        if not self.controlnet_vae:
            raise ValueError(
                "controlnet_vae must be set when using a FLUX Control LoRA."
            )
        ...
        vae_info = context.models.load(self.controlnet_vae.vae)
        return FluxVaeEncodeInvocation.vae_encode(
            vae_info=vae_info, image_tensor=img_cond
        )
    ...
    def _prep_ip_adapter_image_prompt_clip_embeds(
        ...
        for ip_adapter_field in ip_adapter_fields:
            ...
            if isinstance(ip_adapter_field.image, ImageField):
                ...
                raise ValueError(
                    f"Unsupported IP-Adapter image type: {type(ip_adapter_field.image)}"
                )
            ...
            ipa_images = [
                context.images.get_pil(image.image_name, mode="RGB")
                for image in ipa_image_fields
            ]
            ...
            with context.models.load(
                ip_adapter_field.image_encoder_model
            ) as image_encoder_model:
                assert isinstance(image_encoder_model, CLIPVisionModelWithProjection)

                clip_image: torch.Tensor = clip_image_processor(
                    images=pos_images, return_tensors="pt"
                ).pixel_values
                clip_image = clip_image.to(
                    device=device, dtype=image_encoder_model.dtype
                )
                pos_clip_image_embeds = image_encoder_model(clip_image).image_embeds

                clip_image = clip_image_processor(
                    images=neg_images, return_tensors="pt"
                ).pixel_values
                clip_image = clip_image.to(
                    device=device, dtype=image_encoder_model.dtype
                )
    ...
    def _prep_ip_adapter_extensions(
        ...
        for (
            ip_adapter_field,
            pos_image_prompt_clip_embed,
            neg_image_prompt_clip_embed,
        ) in zip(
            ip_adapter_fields,
            pos_image_prompt_clip_embeds,
            neg_image_prompt_clip_embeds,
            strict=True,
        ):
            ip_adapter_model = exit_stack.enter_context(
                context.models.load(ip_adapter_field.ip_adapter_model)
            )
    ...
    def _build_step_callback(
        ...
        def step_callback(state: PipelineIntermediateState) -> None:
            state.latents = unpack(
                state.latents.float(), self.height, self.width
            ).squeeze()
