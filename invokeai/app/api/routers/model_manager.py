...
from invokeai.backend.model_manager.metadata.fetch.huggingface import (
...
def add_cover_image_to_model_config(
    config: AnyModelConfig, dependencies: Type[ApiDependencies]
) -> AnyModelConfig:
...
async def list_model_records(
    ...
    model_name: Optional[str] = Query(
        default=None, description="Exact match on the name of the model"
    ),
    ...
    logger.info(f"record store: {record_store}")
    if base_models:
        ...
        found_models.extend(
            record_store.search_by_attr(
                model_type=model_type, model_name=model_name, model_format=model_format
            )
        )
...
async def get_model_records_by_attrs(
    ...
    if not configs:
        raise HTTPException(
            status_code=404, detail="No model found with these attributes"
        )
...
async def scan_for_models(
    scan_path: str = Query(
        description="Directory path to search for models", default=None
    ),
) -> List[FoundModel]:
    ...
    try:
        ...
        non_core_model_paths = [
            p for p in found_model_paths if not p.is_relative_to(core_models_path)
        ]

        installed_models = (
            ApiDependencies.invoker.services.model_manager.store.search_by_attr()
        )
        ...
        for p in non_core_model_paths:
            path = str(p)
            is_installed = any(
                str(models_path / m.path) == path for m in installed_models
            )
...
async def get_hugging_face_models(
    hugging_face_repo: str = Query(
        description="Hugging face repo to search for models", default=None
    ),
) -> HuggingFaceModels:
    try:
...
async def update_model_record(
    ...
    changes: Annotated[
        ModelRecordChanges,
        Body(description="Model config", example=example_model_input),
    ],
) -> AnyModelConfig:
...
async def install_model(
    ...
    inplace: Optional[bool] = Query(
        description="Whether or not to install a local model in place", default=False
    ),
    access_token: Optional[str] = Query(
        description="access token for the remote resource", default=None
    ),
...
async def install_hugging_face_model(
    ...
    def generate_html(
        title: str, heading: str, repo_id: str, is_error: bool, message: str | None = ""
    ) -> str:
        if message:
    ...
    try:
        ...
        return HTMLResponse(
            content=generate_html(title, heading, source, True, message),
            status_code=400,
        )
    ...
    try:
        ...
        return HTMLResponse(
            content=generate_html(title, heading, source, False, message),
            status_code=201,
        )
    except Exception as e:
        logger.error(str(e))
...
async def list_model_installs() -> List[ModelInstallJob]:
    ...
    * "cancelled" -- Job was cancelled before completion.
    ...
    """
    jobs: List[ModelInstallJob] = (
        ApiDependencies.invoker.services.model_manager.install.list_jobs()
    )
    return jobs
...
async def get_model_install_job(
    id: int = Path(description="Model install id"),
) -> ModelInstallJob:
    """
    ...
    """
    try:
        result: ModelInstallJob = (
            ApiDependencies.invoker.services.model_manager.install.get_job_by_id(id)
        )
        return result
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
async def convert_model(
    ...
    if not isinstance(model_config, MainCheckpointConfig):
        ...
        raise HTTPException(
            400, f"The model with key {key} is not a main checkpoint model."
        )

    with TemporaryDirectory(
        dir=ApiDependencies.invoker.services.configuration.models_path
    ) as tmpdir:
...
def get_is_installed(
    starter_model: StarterModel | StarterModelWithoutDependencies,
    installed_models: list[AnyModelConfig],
) -> bool:
    for model in installed_models:
        ...
        if (
            (
                model.name == starter_model.name
                or model.name in starter_model.previous_names
            )
...
@model_manager_router.get(
    "/starter_models",
    operation_id="get_starter_models",
    response_model=StarterModelResponse,
)
async def get_starter_models() -> StarterModelResponse:
    ...
    return StarterModelResponse(
        starter_models=starter_models, starter_bundles=starter_bundles
    )
...
@model_manager_router.get(
    "/hf_login", operation_id="get_hf_login_status", response_model=HFTokenStatus
)
...
@model_manager_router.post(
    "/hf_login", operation_id="do_hf_login", response_model=HFTokenStatus
)
