...
from invokeai.backend.stable_diffusion.denoise_context import (
    DenoiseContext,
    DenoiseInputs,
)
...
from invokeai.backend.stable_diffusion.extension_callback_type import (
...
from invokeai.backend.stable_diffusion.schedulers.schedulers import (
...
class DenoiseLatentsInvocation(BaseInvocation):
    ...
    cfg_rescale_multiplier: float = InputField(
        ...
        ge=0,
        lt=1,
        description=FieldDescriptions.cfg_rescale_multiplier,
    )
    ...
    def _get_text_embeddings_and_masks(
        ...
        for cond in cond_list:
            ...
            text_embeddings.append(
                cond_data.conditionings[0].to(device=device, dtype=dtype)
            )
    ...
    def get_conditioning_data(
        ...
        cond_text_embeddings, cond_text_embedding_masks = (
            DenoiseLatentsInvocation._get_text_embeddings_and_masks(
        ...
        cond_text_embedding, cond_regions = (
            DenoiseLatentsInvocation._concat_regional_text_embeddings(
    ...
    def prep_control_data(
        ...
        for control_info in control_list:
            control_model = exit_stack.enter_context(
                context.models.load(control_info.control_model)
            )
    ...
    def prep_ip_adapter_image_prompts(
        ...
        for single_ip_adapter in ip_adapters:
            with context.models.load(
                single_ip_adapter.ip_adapter_model
            ) as ip_adapter_model:
                ...
                single_ipa_images = [
                    ...
                    for image in single_ipa_image_fields
                ]
                with context.models.load(
                    single_ip_adapter.image_encoder_model
                ) as image_encoder_model:
                    assert isinstance(
                        image_encoder_model, CLIPVisionModelWithProjection
                    )
                    # Get image embeddings from CLIP and ImageProjModel.
                    image_prompt_embeds, uncond_image_prompt_embeds = (
                        ip_adapter_model.get_image_embeds(
    ...
    def prep_ip_adapter_data(
        ...
        for single_ip_adapter, (image_prompt_embeds, uncond_image_prompt_embeds) in zip(
            ...
            mask = (
                context.tensors.load(mask_field.tensor_name)
                if mask_field is not None
                else None
            )
            mask = self._preprocess_regional_prompt_mask(
                mask, latent_height, latent_width, dtype=dtype
            )
    ...
    def run_t2i_adapters(
        ...
        for t2i_adapter_field in t2i_adapter:
            t2i_adapter_model_config = context.models.get_config(
                t2i_adapter_field.t2i_adapter_model.key
            )
            image = context.images.get_pil(
                t2i_adapter_field.image.image_name, mode="RGB"
            )
            ...
            if t2i_adapter_model_config.base == BaseModelType.StableDiffusion1:
                ...
                raise ValueError(
                    f"Unexpected T2I-Adapter base model type: '{t2i_adapter_model_config.base}'."
                )

            t2i_adapter_model: T2IAdapter
            with context.models.load(
                t2i_adapter_field.t2i_adapter_model
            ) as t2i_adapter_model:
                ...
                t2i_image = prepare_control_image(
                    ...
                    width=control_width_resize,
                    height=control_height_resize,
                    num_channels=t2i_adapter_model.config[
                        "in_channels"
                    ],  # mypy treats this as a FrozenDict
                    device=device,
                ...
                t2i_input_height = (
                    latents_shape[2] // max_unet_downscale * total_downscale_factor
                )
                t2i_input_width = (
                    latents_shape[3] // max_unet_downscale * total_downscale_factor
                )
                if (
                    t2i_image.shape[2] > t2i_input_height
                    or t2i_image.shape[3] > t2i_input_width
                ):
                    t2i_image = t2i_image[
                        :,
                        :,
                        : min(t2i_image.shape[2], t2i_input_height),
                        : min(t2i_image.shape[3], t2i_input_width),
                    ]
    ...
    def prep_inpaint_mask(
        ...
        mask = tv_resize(
            mask, latents.shape[-2:], T.InterpolationMode.BILINEAR, antialias=False
        )
    ...
    def prepare_noise_and_latents(
        context: InvocationContext,
        noise_field: LatentsField | None,
        latents_field: LatentsField | None,
        ...
        if noise is not None and noise.shape[1:] != latents.shape[1:]:
            raise ValueError(
                f"Incompatible 'noise' and 'latents' shapes: {latents.shape=} {noise.shape=}"
            )
    ...
    def _new_invoke(self, context: InvocationContext) -> LatentsOutput:
        ...
        seed, noise, latents = self.prepare_noise_and_latents(
            context, self.noise, self.latents
        )
        ...
        ### inpaint
        mask, masked_latents, is_gradient_mask = self.prep_inpaint_mask(
            context, latents
        )
        ...
        if unet_config.variant == ModelVariantType.Inpaint:
            ext_manager.add_extension(
                InpaintModelExt(mask, masked_latents, is_gradient_mask)
            )
        ...
        with ExitStack() as exit_stack:
            ...
            self.parse_t2i_adapter_field(
                exit_stack, context, self.t2i_adapter, ext_manager, bgr_mode
            )
            ...
            with (
                context.models.load(self.unet.unet).model_on_device() as (
                    cached_weights,
                    unet,
                ),
    ...
    def _old_invoke(self, context: InvocationContext) -> LatentsOutput:
        ...
        image_prompts = self.prep_ip_adapter_image_prompts(
            context=context, ip_adapters=ip_adapters
        )
