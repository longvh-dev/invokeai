...
class SD3DenoiseInvocation(BaseInvocation, WithMetadata, WithBoard):
    ...
    denoising_start: float = InputField(
        default=0.0, ge=0, le=1, description=FieldDescriptions.denoising_start
    )
    ...
    cfg_scale: float | list[float] = InputField(
        default=3.5, description=FieldDescriptions.cfg_scale, title="CFG Scale"
    )
    ...
    def _load_text_conditioning(
        ...
        clip_prompt_embeds = torch.cat(
            [sd3_conditioning.clip_l_embeds, sd3_conditioning.clip_g_embeds], dim=-1
        )
        ...
        pooled_prompt_embeds = torch.cat(
            [
                sd3_conditioning.clip_l_pooled_embeds,
                sd3_conditioning.clip_g_pooled_embeds,
            ],
            dim=-1,
        )
    ...
    def _run_diffusion(
        ...
        with transformer_info.model_on_device() as (cached_weights, transformer):
            ...
            for step_idx, (t_curr, t_prev) in tqdm(
                ...
                timestep = torch.tensor([t_curr * 1000], device=device).expand(
                ...
                if do_classifier_free_guidance:
                    ...
                    noise_pred = noise_pred_uncond + cfg_scale[step_idx] * (
                ...
                if inpaint_extension is not None:
                    latents = (
                        inpaint_extension.merge_intermediate_latents_with_init_latents(
                            latents, t_prev
                        )
                    )
