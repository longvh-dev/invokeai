...
class FluxLoRALoaderOutput(BaseInvocationOutput):
    ...
    clip: Optional[CLIPField] = OutputField(
        default=None, description=FieldDescriptions.clip, title="CLIP"
    )
...
class FluxLoRALoaderInvocation(BaseInvocation):
    ...
    def invoke(self, context: InvocationContext) -> FluxLoRALoaderOutput:
        ...
        if self.transformer and any(
            lora.lora.key == lora_key for lora in self.transformer.loras
        ):
        ...
        if self.t5_encoder and any(
            lora.lora.key == lora_key for lora in self.t5_encoder.loras
        ):
